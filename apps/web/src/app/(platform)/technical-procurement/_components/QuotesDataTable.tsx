    ...
    accessor: (quote) => (
      <div className="flex flex-col gap-1.5">
        <Button
          ...
          size="sm"
          onClick={() => toggleAcceptQuote(quote.id)}
...
export default function QuotesDataTable() {
  const {
    selectedRfq,
    selectedRfqQuotes,
    isLoadingQuotes,
  ...
  const toggleAcceptQuote = async (quoteId: string) => {
    try {
      setAcceptingQuoteId(quoteId);
      // Unaccept the quote. selectedQuoteId=null
      if (selectedRfq?.selectedQuoteId === quoteId) {
        await rfqClient.updateRfq(selectedRfq.id, { selectedQuoteId: null });
        updateRfqLocal({ ...selectedRfq, selectedQuoteId: null });
        return;
      }
      // Accept the quote. selectedQuoteId=quoteId
      if (selectedRfq?.selectedQuoteId !== quoteId) {
        if (!selectedRfq?.id) {
  ...
  const quoteColumns = QuoteDataTableColumns(
  ...
  if (isLoadingQuotes && selectedRfqQuotes.length === 0 && !isRefreshingQuotes) {
    return <LoadingComponent text="Loading quotes..." />;
  }

  // Show "no quotes" message only when not loading/refreshing and no quotes exist
  if (selectedRfqQuotes.length === 0 && !isLoadingQuotes && !isRefreshingQuotes) {
    return (
