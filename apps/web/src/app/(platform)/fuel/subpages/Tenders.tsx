...
const FuelTendersPage = memo(function TendersPage() {
  ...
  const handleGenerateRandomBid = async () => {
    ...
    await fuelBidClient.createFuelBid(
      selectedTender?.id || '',
      newBid as unknown as CreateFuelBidData,
    );
    addBid(newBid as unknown as FuelBid);
      ...
      {currentTender && (
        <div className="flex flex-col gap-4">
          ...
          {loading.bids ? (
            ...
            <BaseCard
              title="Fuel Bids"
              subtitle="Compare and evaluate fuel bids for {selectedTender?.title}"
              icon={<FileText className="w-6 h-6 text-blue-500" />}
              actions={
                <div className="flex gap-2">
                  <FileUploadPopover
                    trigger={<Button intent="add" icon={Plus} text="Add Bid" />}
                    onSend={() => {}}
                  />
                  <Button
                    intent="secondary"
