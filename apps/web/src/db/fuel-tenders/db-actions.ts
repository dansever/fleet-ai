 ...
 */
export async function getFuelTendersByAirportId(airportId: string): Promise<FuelTender[]> {
  const result = await db
    .select()
    .from(fuelTendersTable)
 ...
 */
export async function getFuelTendersByOrgId(orgId: string): Promise<FuelTender[]> {
  const result = await db
    .select()
    .from(fuelTendersTable)
