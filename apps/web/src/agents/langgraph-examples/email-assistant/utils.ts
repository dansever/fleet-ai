...
export function isToolCall(item: any): item is ToolCall {
  return typeof item === 'object' && item !== null && 'name' in item && 'args' in item;
}
  ...
  for (const message of messages) {
    ...
    if (typeof message === 'object' && message !== null) {
      ...
      if (message.tool_calls && Array.isArray(message.tool_calls)) {
        toolCallNames.push(...message.tool_calls.map((call: any) => call.name.toLowerCase()));
      }
      // Handle class instances with toolCalls property
      else if ('toolCalls' in message && Array.isArray(message.toolCalls)) {
        toolCallNames.push(...message.toolCalls.map((call: any) => call.name.toLowerCase()));
      }
    }
  }
