...
import { formatEmailMarkdown, formatForDisplay, parseEmail } from './utils.js';
...
const hasToolCalls = (message: BaseMessage): message is AIMessage & { tool_calls: ToolCall[] } => {
  return (
    message.getType() === 'ai' &&
...
export const initializeHitlEmailAssistant = async () => {
  ...
  const llmCallNode = async (state: EmailAgentHITLStateType, config: LangGraphRunnableConfig) => {
    ...
    const allMessages: BaseMessageLike[] = [{ role: 'system', content: systemPrompt }, ...messages];
