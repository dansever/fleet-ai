// Storage client-side operations
...
export async function uploadFile(file: File, documentType: string) {
  ...
  const { data } = await api.post('/api/file-manager/storage/upload', fd, {
...
export async function listFiles(documentType?: string) {
  const { data } = await api.get('/api/file-manager/storage/list', { params: { documentType } });
...
export async function getSignedUrl(path: string, expiresIn = 60) {
  const { data } = await api.post('/api/file-manager/storage/sign', { path, expiresIn });
...
export async function deleteFile(path: string) {
  await api.delete('/api/file-manager/storage/delete', { data: { path } });
}
